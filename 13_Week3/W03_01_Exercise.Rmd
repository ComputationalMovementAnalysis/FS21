...
Picking up from the previous input: we can now implement this algorithm using the `dplyr` / `mutate()` method. This might seem a little challenging at first, but if you have completed Task 2 from laast week, it is quite easy. You can pass *anything* to a new column within `mutate()` as long as it is a vector of the same length as the original dataframe. 
...
- You can apply the threshold with an `ifelse()`statement withthe condition (`stepMean > 15`), and two keywords when this condition is met (e.g. "move") and when it's not (e.g. "stop").
...
if (knitr::is_html_output()){
  library(leaflet)
  library(scales)
  factpal <- colorFactor(hue_pal()(2), wildschwein_BE$moving)
