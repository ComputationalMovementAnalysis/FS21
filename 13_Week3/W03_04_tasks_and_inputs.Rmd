...
```{r, opts.label="example_showOutput", fig.cap = "The trajectory of sabi, reduced to the positions where the animal was not static"}
sabi <- sabi %>% 
  ungroup() %>%
...
```{r, code =  mydecrypt("13_Week3/solutions/task_2.R",".passphrase"), opts.label="solution_hideOutput"}
```
...
Download and import the dataset `r download_url("pedestrian.csv","00_Rawdata")` (right click > save target as..) as a dataframe (you don't need an `sf` object). It it a set of six different but similar trajectories from pedestrians walking on a path. Explore this data visually.
