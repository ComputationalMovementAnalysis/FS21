...
1. Open the project you created in [the preparation task](#w2-project)
2. Install the package `zoo` to get access to the rolling window functions for the last exercise (`install.packages("zoo")`)
3. Download the wildboar movement data again, and save it to your new project's directory `r download_url("wildschwein_BE.csv","00_Rawdata")` (right click "Save target as..")
4. Create a new R- (or RMarkdown-) file and begin with the following lines of code:
...
wildschwein_BE <- read_delim("00_Rawdata/wildschwein_BE.csv",",") # adjust path
...
Now, commit your changes to your repo like we did [last week](#w1-tasks-and-inputs). To push these changes to Github, click on the green button "Push" in the Git tab in RStudio (this is just to test if the connection to Github works. You won't need to push after every commit).
