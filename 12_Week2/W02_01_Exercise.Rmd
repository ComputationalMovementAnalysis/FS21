...
#### Filter
To do so, filter your data to positions with a sampling interval between 40 and 60 seconds and save it to a new variable (we will use `wildschwein_BE_1`). From this subset, take the first 100 positions[^1] for the following task. 

[^1]: In the solution (and the graphics), we actually use locations 2-100 for aesthetical reasons
...
rm(list = ls())
```
<!-- Todo nils: call R-File here rather than explicitly writing the script -->
