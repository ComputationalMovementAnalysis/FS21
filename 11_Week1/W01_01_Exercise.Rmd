...
<!-- TODO: add an exercise for posixct-->
...
- For everyone working on the RStudio Server: You will not need to upload this data to the server, we've stored the file in a folder accessible to you. The data is saved in a Folder named "CMA_FS2018_Filestorage" one level *above* your *home* folder. You will have to move "up" one or two levels from your project folder and then into the correct folder using the syntax `"../../CMA_FS2018_Filestorage/"`(for two levels) or `"../CMA_FS2018_Filestorage/"` (for one level). 
...
Setting up a `ggplot` with our data is done using the command `ggplot(wildschwein_BE, aes(Lat, Long, colour = TierID))`. Creating a map is done via the basic scatter plot command `geom_point()`, using a fixed aspect ratio of 1 (`coord_fixed(1)`).  Assigning every individual its own colour is done using the `ggplot` argument `colour =`.

Save your plot using `ggsave()`. Save your code in the appropriate section.
...
Notice how `st_as_sf` takes the EPSG code for the `crs =` argument. This is so much easier and more elegant than using `PROJ.4` or `WKT`. You can find a lot of useful information on Coordinate Reference Systems (including EPSG Codes , etc.) under [spatialreference.org](http://spatialreference.org/ref/epsg/2056/).

Let's compare our original `data.frame` with this new `sf` object:

```{r, echo = T, include = T, collapse=F}
wildschwein_BE
...
So what can we do with our new `sf` object that we couldn't before? One example is projecting the WGS84 (`Lat`/`Long`) coordinates into the new Swiss CRS `CH1903+ LV95`[^50]. Do this by using the function `st_transform`. By the way, do you notice a pattern here? The package `sf` names most functions for spatial operations with the prefix `sf_*`.

[^50]: As we've mentioned in the Input 1, you can look up the EPSG codes under [spatialreference.org](http://spatialreference.org/ref/epsg/2056/). 
...
- Oddly, it is not sufficient to specify the `crs = ` within `coord_sf()`, we need to pass our EPSG code to `datum =`. 
...
In order to add a background map to our plot, we will need to load two additional libraries: `raster` to load raster files and `ggspatial` to plot raster files in `ggplot`. Load these libraries now, and import the file `pk100_BE_2056.tif`[^30] using the function `brick()`. Add the newly created `RasterBrick` object to ggplot using the function `geom_spraster_rgb()`.
 
 [^30]: Stored here: `../CMA_FS2018_Filestorage/pk100_BE_2056.tif`
