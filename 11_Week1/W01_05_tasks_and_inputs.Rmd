...
1. make sure you have read and followed the instructions in section [Preperation](#w1-preperation)
...
3. Download the wildboar movement data here: `r download_url("wildschwein_BE.csv","00_Rawdata")` (right click "Save target as..")
4. lastly, create a new R- (or RMarkdown) file and begin by loading the following packages: 

```{r, opts.label = "example_hideOutput"}
...
4. Click in the checkbox left of the file you want to commit (you should find 3 new files: your `.RProj`, `.gitignore` and `.R`-file)
...
Move the file `wildschwein_BE.csv` you just downloaded into your project directory and import it into `r` as a `data.frame`.  Assign correct column types as necessary and make sure the time zone is set correctly for the date/time column.
...
```{r, code =  mydecrypt("11_Week1/solutions/task_1.R",".passphrase"), opts.label="solution_hideOutput"}
```
...
```{r code =  mydecrypt("11_Week1/solutions/task_2.R",".passphrase"), opts.label="solution_showOutput", fig.cap="Your plot should look something like this."}
```
...
```{r, opts.label="example_showOutput", collapse=FALSE}
wildschwein_BE
...
```{r, code =  mydecrypt("11_Week1/solutions/task_3.R",".passphrase"), opts.label="solution_hideOutput"}
```
...
```{r, code =  mydecrypt("11_Week1/solutions/task_4.R",".passphrase"), opts.label="solution_showOutput"}
```
...
In the next task, we would like to add a background map to our `mcp` object. Download the file here: `r download_url("pk100_BE.tif","00_Rawdata")` 
...
```{r, code =  mydecrypt("11_Week1/solutions/task_5.R",".passphrase"), opts.label="solution_showOutput"}
```
...
```{r code =  mydecrypt("11_Week1/solutions/task_6.R",".passphrase"), opts.label="solution_hideOutput"}
```
