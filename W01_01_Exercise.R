...
#' Until now, we've handled spatial data within data frames. This works well for many tasks, but sometimes we need special *spatial* classes to handle our trajectories. We will get to know such cases in our next tasks, but first we need to convert our `data.frame` into a spatial object.
...
#' 
#' In order for `st_convex_hull()` to respect the grouping variable, we need to warp the `sf` object within `summarise()`
#' 
## ---- echo = T,include = T,eval = T--------------------------------------
...
#' 
#' In order to add a background map to our plot, we will need to load two additional libraries: `raster` to load raster files and `ggspatial` to plot raster files in `ggplot`. Load these libraries now, and import the file `pk100_BE_2056.tif` (available on moodle) using the function `brick()`. Add the newly created `RasterBrick` object to ggplot using the function `geom_spraster_rgb()`. Note: 
#' 
#' - `ggplot` layers are added in the order they are called. If you call `geom_spraster_rgb()` last, it will plot over (and thus cover) the `geom_sf()` layer.
